...
const COMPARISON_METRICS: ComparisonMetric[] = [
  { label: 'Total Flow', key: 'total_flow_gpm', unit: 'GPM', higherIsBetter: false },
  { label: 'System Head', key: 'system_head_ft', unit: 'ft', higherIsBetter: false },
  { label: 'Max Velocity', key: 'max_velocity_fps', unit: 'fps', higherIsBetter: false },
  { label: 'Pump Power', key: 'pump_power_hp', unit: 'HP', higherIsBetter: false },
];

export default function PipeComparison() {
  ...
  const { data: systems, isLoading } = usePipeSystems(projectId || undefined);
  ...
  return (
    <DashboardLayout>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-lg font-semibold">Pipe System Comparison</h1>
              </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-80 border-r bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Select up to 4 pipe systems for side-by-side comparison
            </p>
            ...
            <ScrollArea className="h-[calc(100vh-16rem)]">
              <div className="space-y-2">
                {isLoading ? (
                  ...
                  <p className="text-sm text-muted-foreground">No pipe systems found</p>
                ) : (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                              {system.system_type} • {system.total_flow_gpm?.toFixed(0) || 0} GPM
                            </p>
          ...
          <div className="flex-1 p-4 overflow-auto">
            {selectedSystems.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                ...
                <p className="text-sm">Select pipe systems from the left panel to compare</p>
              </div>
            ) : (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          ...
                          <tr className="border-b">
                            <td className="py-2 px-3 text-muted-foreground">Pipe Material</td>
                            {selectedSystems.map(system => (
                              <td key={system.id} className="text-right py-2 px-3 capitalize">
                                {system.pipe_material?.replace('_', ' ') || '-'}
                              </td>
                            ))}
                          </tr>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {selectedSystems.length >= 2 && (
                        <>
                          {(() => {
                            const lowestHead = [...selectedSystems].sort((a, b) => 
                              (a.system_head_ft || 0) - (b.system_head_ft || 0)
                            )[0];
                            return (
                              <div className="flex items-start gap-3 p-3 rounded-lg bg-success/10">
                                <CheckCircle2 className="w-5 h-5 text-success mt-0.5" />
                                <div>
                                  <p className="font-medium text-success">Lowest System Head</p>
                                  <p className="text-sm text-muted-foreground">
                                    <span className="font-medium">{lowestHead.system_name}</span> has the lowest 
                                    system head at {lowestHead.system_head_ft?.toFixed(1)} ft, 
                          ...
                          {(() => {
                            const highVelocity = selectedSystems.filter(s => (s.max_velocity_fps || 0) > 8);
                                ...
                                <div className="flex items-start gap-3 p-3 rounded-lg bg-warning/10">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                      ...
                                      8 fps which may cause noise and erosion issues.
                                    </p>
                                  </div>
                                </div>
                              );
      ...
      <DesignWorkflowNextStep
        currentPath="/design/pipe-comparison"
